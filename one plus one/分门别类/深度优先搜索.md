...
DFS解决的是连通性的问题，即给定一个起始点（起始状态）和一个终点（最终状态），判断是否有一条路径能从起点连接到终点。

很多情况下，连通的路径有很多条，**只需找到一条即可**，DFS**只关心路径存在与否，不在乎其长短。**
...
![img](../img/CgotOV2IkYuANIhxAAB2CBZsYLQ484.png)

### 解题思路

要解决DFS问题，必须依赖栈（Stack），特点是后进先出（LIFO, last in first out）。

第一步，选择一个起始顶点，例如从顶点A开始。把A压入栈，标记他为访问过（用黄色标记），并输出到结果中。

![img](../img/CgoB5l2IkYyAAZhkABsNRVtft9s555.gif)

第二步，寻找与 A 相连并且还没有被访问过的顶点，顶点 A 与 B、D、G 相连，而且它们都还没有被访问过，我们按照字母顺序处理，所以将 B 压入栈，标记它为访问过，并输出到结果中。

![img](../img/CgoB5l2IkYyAHhJWACv4GjTZRBQ760.gif)

第三步，现在我们在顶点 B 上，重复上面的操作，由于 B 与 A、E、F 相连，如果按照字母顺序处理的话，A 应该是要被访问的，但是 A 已经被访问了，所以我们访问顶点 E，将 E 压入栈，标记它为访问过，并输出到结果中。

![img](../img/CgotOV2IkYyAO9LVACEhHBdbKfc149.gif)

第四步，从 E 开始，E 与 B、G 相连，但是B刚刚被访问过了，所以下一个被访问的将是G，把G压入栈，标记它为访问过，并输出到结果中。

![img](../img/CgoB5l2IkYyAc3T6ACUuR_5lcvw842.gif)
...
![img](../img/CgotOV2IkYyAYBQ4AA7A5yscltI499.gif)

可以看到，每次我们在考虑下一个要被访问的点是什么的时候，如果发现周围的顶点都被访问了，就把当前的顶点弹出。

第六步，现在栈的顶部记录的是顶点 E，我们来看看与 E 相连的顶点中有没有还没被访问到的，发现它们都被访问了，所以把 E 也弹出去。

![img](../img/CgoB5l2IkYyAe__KABAUImf6ENE708.gif)

第七步，当前栈的顶点是 B，看看它周围有没有还没被访问的顶点，有，是顶点 F，于是把 F 压入栈，标记它为访问过，并输出到结果中。

![img](http://s0.lgstatic.com/i/image2/M01/91/43/CgotOV2IkY2ACNi3AEWj2_BWcsM296.gif)

第八步，当前顶点是 F，与 F 相连并且还未被访问到的点是 C 和 D，按照字母顺序来，下一个被访问的点是 C，将 C 压入栈，标记为访问过，输出到结果中。

![img](http://s0.lgstatic.com/i/image2/M01/91/43/CgotOV2IkY2ABY_CAD5yLy-V6CM016.gif)

第九步，当前顶点为 C，与 C 相连并尚未被访问到的顶点是 H，将 H 压入栈，标记为访问过，输出到结果中。

![img](http://s0.lgstatic.com/i/image2/M01/91/23/CgoB5l2IkY2AHiebAEZJ2pzOuiQ289.gif)

第十步，当前顶点是 H，由于和它相连的点都被访问过了，将它弹出栈。

![img](http://s0.lgstatic.com/i/image2/M01/91/43/CgotOV2IkY6AMrbOAC9gCtQSDyg193.gif)

第十一步，当前顶点是 C，与 C 相连的点都被访问过了，将 C 弹出栈。

![img](../img/CgotOV2IkZCAb7O6ACq7mfesvtU046.gif)

第十二步，当前顶点是 F，与 F 相连的并且尚未访问的点是 D，将 D 压入栈，输出到结果中，并标记为访问过。

![img](../img/CgoB5l2IkZGAcNsZACm2oC7I53I299.gif)

第十三步，当前顶点是 D，与它相连的点都被访问过了，将它弹出栈。以此类推，顶点 F，B，A 的邻居都被访问过了，将它们依次弹出栈就好了。最后，当栈里已经没有顶点需要处理了，我们的整个遍历结束。

![img](../img/CgoB5l2IkZKAUYofADryI0IEla8177.gif)

例题分析一
给定一个二维矩阵代表一个迷宫，迷宫里面有通道，也有墙壁，通道由数字 0 表示，而墙壁由 -1 表示，有墙壁的地方不能通过，那么，能不能从 A 点走到 B 点。

![img](../img/CgoB5l2IkZOAZAaTAAEnEYY55UA254.png)        

从 A 开始走的话，有很多条路径通往 B，例如下面两种。

![img](../img/CgotOV2IkZmAUAsQAOl9ssa2zxE177.gif)

![img](../img/CgotOV2IkZ6AEp2BAOF4o1jndN0409.gif)
...
- 看上去简洁
- 实际应用中，递归需要压入和弹出栈，栈深的时候会造成运行效率底下。
...
![img](http://s0.lgstatic.com/i/image2/M01/91/43/CgotOV2IkaSAJZTIAOVn4eGgEXc393.gif)

从各方向到达该点所需要的步数都更多，则不再尝试。

![img](../img/CgotOV2IkamAazS1ANB4kNxFNT4453.gif)
...
![image-20210322160150307](../img/image-20210322160150307.png)

![image-20210322160217347](../img/image-20210322160217347.png)
...
![img](../img/CgoB5l2IkamAcHHCAAB2vDyOBsk961.png)
...
![image-20210322161614867](../img/image-20210322161614867.png)
...
C(K, 0) + C(K, 1) + C(K, 2) + C(K, 3) = 1 + K + K ×(K - 1) / 2 + K× (K - 1) ×(K - 2) / 6
 ...
 ![image-20210322161859519](../img/image-20210322161859519.png)
...
![img](../img/CgoB5l2IkcCAWjRjALszKfUEV7A310.gif)

![img](../img/CgoB5l2IkcOAd-giAItzJVPUNUM375.gif)

![img](../img/CgotOV2IkcaAI78RAHjv2Tul3JY991.gif)
...
![image-20210322163604114](../img/image-20210322163604114.png)
